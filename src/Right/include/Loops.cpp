...
const String rightId = "2222";

void ackLoop(){
    if (!ackReceived && (millis() - lastSent >= interval)) {
        String ackString = "Right:" + String(rightId);
        client.publish(rightAckTopic, ackString.c_str());  // Send ID to server
        Serial.println("Broadcasting init to server...");
        Serial.println(String(ackReceived));
        Serial.println((String)rightAckTopic + " " + rightId);
        lastSent = millis();
    }
}
