...
double sliderPosition = 0; // Starting from 0 (silent)
double subSliderPosition = 0.8;
float currentVolumeDb = -50.0;
float currentSubVolumeDb = -12.00;
// Constants for the exponential curve
const double a = 0.001;
const double b = 6.908;

// Volume control function
float volumeControl(double position) {
    ...
    if (position < 0.1) {
        amplitudeRatio = position * 10 * a * exp(0.1 * b);
    } else {
        // Use exponential curve for positions 0.1 and above
        amplitudeRatio = a * exp(b * position);
    }

    // Convert amplitude ratio to decibels
    return (float)(20 * log10(amplitudeRatio));
}
    ...
    do {
        ...
        switch (code) {
            case volumeUpCode:
                ...
                volume(volumeControl(sliderPosition));
                break;
            case volumeDownCode:
                lastIRCode = code;
